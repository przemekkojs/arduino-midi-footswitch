...
/// Number of buttons working like push buttons. Push buttons will use pins from A5 backwards. Default value is 4, so pins A2-A5 work like that.
const int NUM_OF_PUSH_BUTTONS = 4; // <<< Change only this number. DO NOT EXCEED NUM_OF_BUTTONS CONSTANT!
    ...
    for (int index = 0; index < NUM_OF_BUTTONS; index++) {
        ...
        if ((millis() - BOUNCE_TIME) > debounceTimes[index]) {  
            if (reading != currentStates[index]) {
                ...
                if (index >= (NUM_OF_BUTTONS - NUM_OF_PUSH_BUTTONS)) {                
                    /*  EXPLANATION
                        Sending one signal for Setzer+ (note 123, button A2, 12) and Setzer- (note 124, button A3, 13) - it should behave like piston, not ON/OFF switch.

                        15 = button A5 = note 126 = G.C. - Grand Cancel - turns off all voices, so all "normal" buttons need to be turned off
                        14 = button A4 = note 125 = TT - Tutti, so all couplers and anches need to be set ON

                        Couplers: 112 - 114, 116 - 118
                        Anches: 115, 120 - 122
                    */                
                    if (index == 14 || index == 15) {
                        for (int pinIndex = 111; pinIndex <= 122; pinIndex++)
                            pressed[pinIndex - 111] = (index == 14); //For 14 true, for 15 false
                    }               
                ...
                else if (currentStates[index] == LOW) { // Other buttons work normally - like switches
